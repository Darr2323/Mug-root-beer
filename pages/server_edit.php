...
if(User::x_to_y('server_id', 'user_id', $_GET['server_id'], 'servers') != $account_user_id){
...
if(isset($_GET['type']) && empty($_POST)) {
	...
	if(empty($_SESSION['error'])) { 
		...
		if($_GET['type'] == 'private') {
			$server->new_private = ($server->data->private) ? 0 : 1;
			$database->query("UPDATE `servers` SET `private` = {$server->new_private} WHERE `server_id` = {$server->data->server_id}");
		}
...
<form action="" method="post" role="form" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<textarea name="votifier_public_key" class="form-control" rows="6"><?php echo $server->data->custom->votifier_public_key; ?></textarea>
	</div>
	...
	<div class="form-group">
		...
		<input type="text" name="votifier_ip" class="form-control" value="<?php echo $server->data->custom->votifier_ip; ?>" />
	</div>

	<div class="form-group">
		<label><?php echo $language['forms']['server_votifier_port']; ?></label>
		<input type="text" name="votifier_port" class="form-control" value="<?php echo $server->data->custom->votifier_port; ?>" />
	</div>
	...
	<div class="form-group">
		...
		<a href="edit-server/<?php echo $_GET['server_id']; ?>/private/<?php echo $token->hash; ?>" >
			<button type="button" class="btn btn-warning"><?php echo ($server->data->private) ? $language['forms']['server_remove_private'] : $language['forms']['server_make_private']; ?></button>
		</a>
